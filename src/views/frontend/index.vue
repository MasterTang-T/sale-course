<template>
  <div class="container mx-auto px-6 py-12">
    <div class="flex justify-end items-center mb-12">
      ...
      >
        <i class="fas fa-shopping-cart text-gray-600 group-hover:text-primary transition-colors duration-300"></i>
        <span class="ml-2 text-gray-600 group-hover:text-primary text-sm font-medium transition-colors duration-300">购物车</span>
        <div v-if="cartItemCount > 0" class="absolute -top-2 -right-2 w-5 h-5 bg-primary text-white text-xs rounded-full flex items-center justify-center">
          {{ cartItemCount }}
        </div>
      </router-link>
    </div>
